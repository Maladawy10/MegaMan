...
public class B2WorldCreator {
	...
	public B2WorldCreator(World world, TiledMap map)
	{
		...
		for(MapObject object:map.getLayers().get(4).getObjects().getByType(RectangleMapObject.class))//2 is index for ground layer after background and graphics
		{
			...
			body.createFixture(fdef);
                ...
                for(MapObject object:map.getLayers().get(5).getObjects().getByType(RectangleMapObject.class))//2 is index for ground layer after background and graphics
		{
			Rectangle rect = ((RectangleMapObject) object).getRectangle();//GET RECTANGLE OBJECT ITSELF
			bdef.type=BodyDef.BodyType.StaticBody;//define bodydef
			bdef.position.set((rect.getX()+rect.getWidth()/2)/MegaMan.PPM,(rect.getY()+rect.getHeight()/2)/MegaMan.PPM);
